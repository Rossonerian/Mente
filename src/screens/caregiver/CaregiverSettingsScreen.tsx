...
function ConnectedCaregiverSettingsScreen({ onOpenSetup, onSwitchRole, caregiverId, accessToken, onSessionExpired }: ConnectedScreenProps & { onSessionExpired: () => void }) {
  ...
  if (settings.kind === 'offline') {
    if (!settings.staleData) return <SettingsMessageState title="You’re offline" body="Reconnect to load and change these settings." actionLabel="Try again" onAction={settings.retry} />;
    return <SettingsForm data={settings.staleData} isRefreshing={false} isStale onRefresh={settings.retry} loadError="You’re offline. Your last saved settings are shown; reconnect before saving changes." onOpenSetup={onOpenSetup} onSwitchRole={onSwitchRole} caregiverId={caregiverId} accessToken={accessToken} />;
  }
  if (settings.kind === 'error') {
    if (!settings.staleData) return <SettingsMessageState title="We could not load settings" body={settings.error.message} actionLabel="Try again" onAction={settings.retry} />;
    return <SettingsForm data={settings.staleData} isRefreshing={false} isStale onRefresh={settings.retry} loadError="We could not refresh these settings. The last saved values are shown; review them before saving." onOpenSetup={onOpenSetup} onSwitchRole={onSwitchRole} caregiverId={caregiverId} accessToken={accessToken} />;
  }
...
function SettingsForm({ data, isRefreshing, isStale, onRefresh, loadError, onOpenSetup, onSwitchRole, caregiverId, accessToken }: { data: CaregiverSettingsViewModel; isRefreshing: boolean; isStale: boolean; onRefresh: () => void; loadError?: string; onOpenSetup: () => void; onSwitchRole: () => void; caregiverId: string; accessToken: string }) {
  ...
  return (
    <ScreenScroll theme="caregiver">
      ...
      {loadError ? <SoftPanel theme="caregiver" style={styles.errorPanel}><Text accessibilityLiveRegion="polite" style={styles.errorText}>{loadError}</Text><MenteButton label="Try again" onPress={onRefresh} theme="caregiver" variant="quiet" /></SoftPanel> : isStale && !isRefreshing ? <SoftPanel theme="caregiver" style={styles.stalePanel}><Text style={styles.staleText}>This information may be out of date.</Text><MenteButton label="Refresh" onPress={onRefresh} theme="caregiver" variant="quiet" /></SoftPanel> : null}
...
const styles = StyleSheet.create({
  ...
  dayField: { gap: spacing.xs, minHeight: 104, paddingVertical: spacing.xs }, dayRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }, dayButton: { alignItems: 'center', backgroundColor: caregiverTheme.colors.surfaceMuted, borderColor: caregiverTheme.colors.border, borderRadius: 8, borderWidth: 1, height: 48, justifyContent: 'center', width: 48 }, dayButtonSelected: { backgroundColor: caregiverTheme.colors.primary, borderColor: caregiverTheme.colors.primary }, dayButtonFocused: { borderColor: caregiverTheme.colors.text, borderWidth: 3 }, dayLabel: { color: caregiverTheme.colors.primary, fontSize: 13, fontWeight: '800' }, dayLabelSelected: { color: caregiverTheme.colors.white },
